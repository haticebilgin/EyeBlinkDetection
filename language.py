...
while (True):
    ...
    frame = imutils.resize(frame, width=360)  # cozunurluk degıstırdık
    ...
    cv2.imshow("Language selection", frame)
    key = cv2.waitKey(50) & 0xff

if total == 0:
    language=1 #turkce devam
else:
    language=0 #ingilizce
cap.release()
...
while (True):
    ...
    for face in faces:
        landmarks = predictor(gray, face)
        ear = eye_blink(landmarks)
        if ear < 0.30:  #goz kapali olabilir
            count += 1
...
if language==1 :
    ...
    if total==0:
        ...
        print("more than 10, {} total blink".format(total))
